...
\author{
    ...
    \textit{Instituto Tecnológico de Buenos Aires (ITBA)} \\[0.5cm]
    Autores: \\
...
El software implementa un motor de cálculo vectorial que sigue el proceso físico de radiación, operando primero con amplitudes de campo complejo (fasores) y convirtiéndolos posteriormente a densidad de potencia para la integración volumétrica. A continuación se detalla la cadena de procesamiento matemático.
...
Las siguientes expresiones corresponden al Patrón de Campo Normalizado, denotado como $f(\theta)$:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \texttt{Phase ($\beta$):} Fase progresiva en grados. Permite realizar escaneo electrónico del haz principal.
    ...
    \end{itemize}
    \item \texttt{Type:} Elemento base (Isotrópica, Dipolo, Monopolo).
    \item \texttt{Array Axis:} Define la geometría del arreglo (Z o X). Esto altera drásticamente la simetría 3D del patrón resultante.
\end{itemize}
...
\begin{itemize}
    \item \textit{View:} Permite alternar entre cortes 2D (Vertical XZ, Horizontal XY) o \texttt{Both} para visualización simultánea.
    \item \textit{Plot:} Representación en coordenadas Polares o Cartesianas.
    \item \textit{3D Orientation:} Activa un gráfico vectorial incrustado que muestra la disposición física de los dipolos y el plano de corte activo en tiempo real.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item $D_{max}$ (dBi): Directividad máxima calculada por integración esférica.
        \item $HPBW$ ($^\circ$): Ancho del lóbulo principal a -3dB.
    \end{itemize}
\end{itemize}
